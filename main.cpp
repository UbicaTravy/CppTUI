...
int main() {
    ...
    while (true) {
		...
		if (width != prevWidth || height != prevHeight) {
			...
			std::string nameGame = "GuessTheNumber";
			int nameGameX = (width - static_cast<int>(nameGame.length())) / 2;
			show_text(nameGame, nameGameX, 1);
        ...
        if (_kbhit()) {
            ...
            if (ch == 224 || ch == 0) { 
				...
				show_text(nameGame, nameGameX, 1);
				show_text(ENGINE_INFO, (width - static_cast<int>(ENGINE_INFO.length())) / 2, height - 2);
				show_text(COMPANY_INFO, (width - static_cast<int>(COMPANY_INFO.length())) / 2, height - 1);
				setColor(15, 1);
            } else if (ch == '\r') { // Enter key
